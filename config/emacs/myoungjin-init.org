...
#+BEGIN_SRC emacs-lisp
  ...
  'doom-modeline 'spaceline+eyeliner)"

    :type '(choice
            (const :tag "Powerline" 'powerline)
            (const :tag "Powerline" 'powerline+airline)
            (const :tag "Doom"      'doom-modeline)
            (const :tag "Spaceline" 'spaceline+eyeliner))
    :group 'work-mode)
...
#+BEGIN_SRC emacs-lisp
  (use-package paredit
    ...
    (progn
      ...
      (global-set-key (kbd "C-c <left>") #'paredit-backward-slurp-sexp) ; WORKAROUND not working with :bind keyward
...
#+BEGIN_SRC emacs-lisp
    ...
    ;; XXX: not working
    ;;    (eval-after-load package-string
    ;;      `(diminish (quote ,mode-string)))
    ;; WORKAROUND
        (add-hook (intern (concat mode-string "-hook"))
                  `(lambda () (diminish (intern ,mode-string))))
        ))
  ;; use alternative names ...
    (add-hook 'paredit-mode-hook (lambda () (diminish 'paredit-mode " ⸨P⸩")))

  ;; but this one working with eval-after-load
...
*** TODO Use defcustom for powerline theme (default, airline:(with theme name))
